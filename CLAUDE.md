...
- **`popup.js`** — Runs when the popup opens. Polls every 5 seconds (`REFRESH_MS`), renders stats, and three tabs: recent blocked queries (with click-to-whitelist), recent allowed queries (with clickable domains and click-to-blacklist), and top blocked domains today (with count bars and click-to-whitelist). Also handles suspend/resume via the Pi-hole API.
